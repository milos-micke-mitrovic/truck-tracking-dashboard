...
import { DRIVER_DOCUMENT_TYPES } from '../../../constants'
...
export function DocumentsTab() {
  ...
  const { control, setValue, watch } = useFormContext<DriverFormValues>()
    ...
    <>
          ...
          <Table className="min-w-[480px]">
            <TableHeader className="bg-muted/50">
              <TableRow className="hover:bg-muted/50">
                ...
                <TableHead>{t('driverDialog.documentType')}</TableHead>
                <TableHead>{t('driverDialog.expirationDate')}</TableHead>
                <TableHead className="w-20">
                  {t('driverDialog.actions')}
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
                      ...
                      <FormField
                        ...
                        render={({ field: typeField }) => (
                          <Select
                            ...
                            placeholder={t('driverDialog.selectDocumentType')}
                          />
                        )}
                      />
                    </TableCell>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        {hasFile ? (
                          <IconButton
                            ...
                            aria-label={t('driverDialog.viewPdf')}
                          />
                        ) : (
                          <IconButton
                            type="button"
                            ...
                            aria-label={t('driverDialog.uploadPdf')}
                          />
                        )}
                        <IconButton
                          type="button"
                          ...
                          aria-label={t('driverDialog.delete')}
      ...
      >
        <DialogContent showCloseButton={false} className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t('driverDialog.deleteConfirmTitle')}</DialogTitle>
          </DialogHeader>
          <div className="py-2">
            <BodySmall className="text-muted-foreground">
              {t('driverDialog.deleteConfirmMessage')}
            </BodySmall>
            {deleteConfirm.hasFile && (
              <div className="mt-4 flex items-center gap-2">
                <Checkbox
                  id="remove-whole-document-driver"
                ...
                />
                <Label
                  htmlFor="remove-whole-document-driver"
                  className="cursor-pointer text-sm"
                >
                  {t('driverDialog.removeWholeDocument')}
                </Label>
              </div>
            )}
          </div>
